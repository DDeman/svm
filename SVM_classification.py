...
class SVM__smo_simple():
    ...
    def calculate_E(self,x_array,y_array,alpha,b):
        pass
    ...
    def fit(self,data,C,toler,max_iters):
        ...
        while iters < max_iters:
            for i in range(m):
                E_i = sum(np.dot(x_array,x_array[i]) * y_array * alpha) + b - y_array[i]
                # print((E_i))
                # b = alpha[i]
                if ((y_array[i] * E_i < - toler) and (alpha[i] < C)) or ((y_array[i] * E_i > toler) and (alpha[i] > 0)):
                    ...
                    if L == H:
                        continue
                    eta = np.dot(x_array[i],x_array[i]) + np.dot(x_array[j],x_array[j]) - 2 * np.dot(x_array[i],x_array[j])
                    if eta <= 0:
                        continue
                    ...
                    bi_new = b - E_i - y_array[i] * (alpha_i_new - alpha[i]) * np.dot(x_array[i],x_array[i]) - y_array[j] * (alpha_j_new - alpha[j]) * np.dot(x_array[j],x_array[i])
                    bj_new = b - E_j - y_array[i] * (alpha_i_new - alpha[i]) * np.dot(x_array[i],x_array[j]) - y_array[j] * (alpha_j_new - alpha[j]) * np.dot(x_array[j],x_array[j])
    ...
    def select_i(self,alpha,y_array,x_array,C,b):
        for idx in range(alpha):
            gx = alpha * y_array * np.dot(alpha[idx],x_array) + b
            if alpha[idx] == 0:
                if y_array[idx] * gx < 1:
                    return idx
            elif 0 < alpha[idx] < C:
...
if __name__ == '__main__':
    ...
    svm = SVM__smo_simple()
    w,b = svm.fit(data_pd,0.6,0.001,1000)
    x_array = np.array(x_pd)
